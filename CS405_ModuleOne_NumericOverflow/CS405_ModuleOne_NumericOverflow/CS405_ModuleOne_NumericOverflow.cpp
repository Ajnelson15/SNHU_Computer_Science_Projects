...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // If adding 'increment' would exceed the max value, we return the max
        if (increment > static_cast<T>(0) && result > (std::numeric_limits<T>::max() - increment))
        {
            // Indicates an overflow condition was detected
            return std::numeric_limits<T>::max();
        }
        // If safe, add 'increment' to 'result'
        result += increment;
    }
    // Return the final value if we never exceeded the numeric limit
    return result;
}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // If subtracting decrement would go below the lowest possible value, return lowest
        if (decrement > static_cast<T>(0) && result < (std::numeric_limits<T>::lowest() + decrement))
        {
            // Indicate an underflow condition
            return std::numeric_limits<T>::lowest();
        }
        // If safe, subtract 'decrement' from 'result'
        result -= decrement;
    }
    // Return final result if no underflow was detected
    return result;
}
...
void test_overflow()
{
    ...
    // If the function indicates overflow, it returns a warning
    if (result == std::numeric_limits<T>::max())
    {
        // Warning message
        std::cout << "Overflow detected (Result = " << +result <<")\n" << std::endl;
    }
    else
    {
        // Otherwise, just show the resulting sum
        std::cout << +result << "\n" << std::endl;
    }
}
...
void test_underflow()
{
    ...
    // If the function indicates underflow, it returns min()
    if (result == std::numeric_limits<T>::lowest())
    {
        std::cout << "Underflow detected (Result = " << +result << ")\n" << std::endl;
    }
    else
    {
        std::cout << +result << "\n" << std::endl;
